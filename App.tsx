...
import Dashboard from './Dashboard'; // Importando da raiz
import DeploymentForm from './DeploymentForm'; // Importando da raiz
import DeploymentList from './DeploymentList'; // Importando da raiz
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans max-w-md mx-auto shadow-2xl relative overflow-hidden flex flex-col">
      {view !== 'FORM' && (
        <header className="bg-slate-800 p-4 sticky top-0 z-10 flex justify-between items-center border-b border-slate-700">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-sky-600 rounded-lg flex items-center justify-center text-white font-bold text-lg shadow-lg">N</div>
            <h1 className="font-bold text-lg tracking-tight text-white">{APP_NAME}</h1>
          </div>
          <button onClick={toggleRole} className={`text-xs font-medium px-3 py-1.5 rounded-full border transition-all ${userRole === USER_ROLES.AUXILIAR ? 'bg-slate-700 text-slate-400 border-slate-600' : 'bg-sky-900 text-sky-300 border-sky-700'}`}>
      ...
      {view !== 'FORM' && (
        <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-slate-800 border-t border-slate-700 px-6 py-3 flex justify-between items-center z-20">
          <button onClick={() => setView('DASHBOARD')} className={`flex flex-col items-center gap-1 ${view === 'DASHBOARD' ? 'text-sky-400' : 'text-slate-500'}`}>
            <Home className="w-6 h-6" />
            <span className="text-[10px] font-medium">Início</span>
          </button>
          <button onClick={() => setView('FORM')} className="bg-sky-600 text-white p-4 rounded-full -mt-12 shadow-lg hover:bg-sky-500 border-4 border-slate-900">
            <Plus className="w-8 h-8" />
          </button>
          <button onClick={() => setView('LIST')} className={`flex flex-col items-center gap-1 ${view === 'LIST' ? 'text-sky-400' : 'text-slate-500'}`}>
